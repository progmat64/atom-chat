...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        channel = await sync_to_async(Channel.objects.get)(name=self.channel_name)
        sender = await sync_to_async(User.objects.get)(username=username)

        await sync_to_async(Message.objects.create)(channel=channel, sender=sender, content=message)
